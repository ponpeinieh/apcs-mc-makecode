...
接著我們要建構第一根柱子。
將 ``||Agent:Agent 放置 [前]||`` 代碼方塊拖放到編碼工作區中，將方向設為**下**方。
...
## 步驟 4
拖放另一個 ``||Agent:Agent 放置 [前]||`` 代碼方塊，將方向設為**下**方。
...
## 步驟 6
再拖放另一個 ``||Agent:Agent 放置 [前]||`` 代碼方塊，將方向設為**下**方。
...
## 步驟 14
更改 ``||positions:世界 [0] [0] [0]||`` 座標中的數字，將其設置為你希望文字開始的位置。在我們的代碼範例中，位置是 **-31, 83, -560**。在輸入這些數字時，請注意有些數字是負的。
...
## 步驟 15
更改 ``||Blocks:方位||`` 的選項，來設定文字顯示的方向。在我們的代碼範例中，方向為**北 (North)**方。
...
## 步驟 16
測試你的代碼。在聊天功能中輸入 **zoo_sign**。你將看到 **ZOO** 這個字串出現在動物園門的上空中。
