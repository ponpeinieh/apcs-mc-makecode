...
## 步驟5
用``||Positions:POSITIONS||``菜單中的``||Positions:世界 [0] [0] [0]||``座標橢圓替換所有三個**相對**座標橢圓。
...
## 步驟9
輸入克隆座標。返回MakeCode，仔細輸入第一、第二和第三組葉片的座標到``||Blocks:從克隆||``代碼塊中的``||Blocks:from||``和``||Blocks:to||``部分。暫時將``||Blocks:into||``設置為空白。
...
## 步驟12
然後將這些添加到所有三個``||Blocks:從克隆||``代碼塊的``||Blocks:into||``座標中。
